...
export function getFarmsPrices(
  ...
  const commonPriceFarms = farms.map((farm) => {
    ...
    if (
      ...
      farm.token.chainId in WNATIVE &&
      farm.token.equals(WNATIVE[farm.token.chainId as keyof typeof WNATIVE])
    ) {
      tokenPriceBusd = new BN(cakePriceUSD)
    }
    if (
      quoteTokenPriceBusd.isZero() &&
      farm.quoteToken.chainId in WNATIVE &&
      farm.quoteToken.equals(WNATIVE[farm.quoteToken.chainId as keyof typeof WNATIVE])
    ) {
