...
router.get('/apr', async ({ query }) => {
  if (typeof query?.key === 'string' && query.key === FORCE_UPDATE_KEY) {
    try {
      const result = await Promise.allSettled(farmFetcher.supportedChainId.map((id: any) => saveLPsAPR(id)))
      return json(result.map((r: any) => r))
    } catch (err) {
      error(500, { err })
    }
  }
...
async function handleScheduled(event: ScheduledEvent) {
  switch (event.cron) {
    case '*/1 * * * *':
    case '*/2 * * * *': {
      const result = await Promise.allSettled(farmFetcher.supportedChainId.map((id: any) => saveFarms(id, event)))
      console.log(result.map((r: any) => r))
      return result
    }
    case '0 0 * * *': {
      const result = await Promise.allSettled(farmFetcher.supportedChainId.map((id: any) => saveLPsAPR(id)))
